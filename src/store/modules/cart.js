...
export default {
  ...
  mutation: {
    insertCart(state, goods) {
      ...
      // 插入数据规则 1、查找相同商品 2、有则查询数量，累加到payload上，再保存最新位置,删除原来商品 3、没有，则保存在最新位置
      if (sameIndex >= 0) {
  ...
  action: {
    ...
    // 修改sku规格函数
    updateCartSku(ctx, { oldSkuId, newSkuId }) {
      return new Promise((resolve, reject) => {
        if (ctx.rootState.user.profile.token) {
          // 登录 TODO
          ...
          // 1. 获取旧的商品信息
          const oldGoods = ctx.state.list.find(item => item.skuId === oldGoods.skuId);
          // 2. 删除旧的商品
    ...
    async mergeLocalCart(ctx) {
      ...
      await this.mergeLocalCart(cartList);
      // 合并成功将本地购物车删除
